...
const View: React.FC = () => {
  ...
  const groupedItem: GroupedItem = useSelector((reduxState: RootState) => {
    return selectedColor.groupedItemId
      ? reduxState.items.groupedItemMap[selectedColor.groupedItemId]
      : ({} as GroupedItem);
  });
  ...
  const setSize = (size: string) => {
    if (availableSizes.includes(size)) {
      setSelectedItem(
        selectedColor.items.find((item: Item) => item.size === size),
      );
    }
  };
  ...
  const sizeList = Array.from(new Set(availableSizes));
