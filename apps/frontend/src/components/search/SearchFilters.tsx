...
type ExpandableFilterKey =
  | ItemFilterKey.Color
  | ItemFilterKey.Type
  | ItemFilterKey.Gender
  | ItemFilterKey.Size;

interface State {
  accordionExpandMap: Record<ExpandableFilterKey, boolean>;
}
...
const SearchFilters: React.FC<Props> = () => {
  ...
  const handleOpen = (key: ExpandableFilterKey) => {
    ...
    updatedFilters.forEach((updatedFilter: ItemFilter) => {
      if (updatedFilter.key === filterKey) {
        const selectedValueIndex: number =
          updatedFilter.selectedValues.indexOf(selectedOption);
  ...
  return (
    <div className="max-h-full overflow-y-auto">
      {filters.map((filter) => (
        <Accordion
          key={filter.key}
          open={state.accordionExpandMap[filter.key as ExpandableFilterKey]}
          icon={<Icon open={state.accordionExpandMap[filter.key as ExpandableFilterKey]} />}
        >
          <AccordionHeader
            className="p-2"
            onClick={() => handleOpen(filter.key as ExpandableFilterKey)}
          >
            {filter.key}
          </AccordionHeader>
