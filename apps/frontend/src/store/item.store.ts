...
const itemsSlice = createSlice({
  ...
  reducers: {
    addGroupedItems: (state, action: PayloadAction<GroupedItem[]>) => {
      ...
      const filterOptionsMap: Record<ItemFilterKey, Set<string>> = {
        [ItemFilterKey.Color]: new Set<string>(),
        [ItemFilterKey.Type]: new Set<string>(),
        [ItemFilterKey.Gender]: new Set<string>(),
        [ItemFilterKey.Size]: new Set<string>(),
        [ItemFilterKey.Search]: new Set<string>(),
        [ItemFilterKey.Price]: new Set<string>(),
      };

      const searchOptionsMap: Record<SearchOptionKey, string[]> = {
        [SearchOptionKey.Name]: [],
        [SearchOptionKey.Type]: [],
        [SearchOptionKey.Text]: [],
      };

      groupedItems.forEach((groupedItem: GroupedItem) => {
        state.groupedItemMap[groupedItem.id] = groupedItem;
        filterOptionsMap[ItemFilterKey.Type].add(groupedItem.type);
        filterOptionsMap[ItemFilterKey.Gender].add(groupedItem.gender);
        ...
        if (
          !searchOptionsMap[SearchOptionKey.Type].includes(groupedItem.type)
        ) {
        ...
        groupedItem.itemColors.forEach((itemColor: ItemColor) => {
          ...
          filterOptionsMap[ItemFilterKey.Color].add(itemColor.color);
          itemColor.items.forEach((item: Item) => {
            state.itemMap[item.id] = item;
            filterOptionsMap[ItemFilterKey.Size].add(item.size);
          });
        });
      });

      state.filters.forEach((filter) => {
        filter.options = Array.from(filterOptionsMap[filter.key]).sort((a, b) =>
          a.localeCompare(b),
        );
      });
      ...
      const sortOrder: Record<SearchOptionKey, number> = {
        [SearchOptionKey.Type]: 0,
        [SearchOptionKey.Name]: 1,
        [SearchOptionKey.Text]: 2,
      };
    ...
    updateFilters(
      ...
      state.filteredItems = Object.values(state.groupedItemMap)
        .map((item: GroupedItem) => JSON.parse(JSON.stringify(item)))
        .filter((groupedItem: GroupedItem) => {
          groupedItem.itemColors = groupedItem.itemColors.filter(
            (itemColor: ItemColor) => {
                ...
                switch (filter.key) {
                  case ItemFilterKey.Color:
                    if (
                      ...
                      !filter.selectedValues.includes(itemColor.color)
                    ) {
                ...
                state.filters.forEach((filter: ItemFilter) => {
                  switch (filter.key) {
                    case ItemFilterKey.Size:
                      if (
                        filter.selectedValues.length > 0 &&
                        !filter.selectedValues.includes(item.size)
                      ) {
          ...
          state.filters.forEach((filter: ItemFilter) => {
            switch (filter.key) {
              case ItemFilterKey.Type:
                if (
                  filter.selectedValues.length > 0 &&
                  !filter.selectedValues.includes(groupedItem.type)
                ) {
                  unfilteredItem = false;
                }
                break;
              case ItemFilterKey.Gender:
                if (
                  filter.selectedValues.length > 0 &&
                  !filter.selectedValues.includes(groupedItem.gender)
                ) {
                  unfilteredItem = false;
                }
                break;
            ...
            switch (searchOption.key) {
              case SearchOptionKey.Text:
                if (
                  ...
                  !containsCaseInsensitive(searchOption.value, groupedItem.type)
                ) {
                  unfilteredItem = false;
                }
                break;
