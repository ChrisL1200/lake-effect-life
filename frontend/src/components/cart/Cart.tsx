...
const Cart: React.FC = () => {
  const populatedCartItems: PopulatedCartItem[] = useSelector(
    (reduxState: RootState) => {
      return reduxState.cart.cartItems.map((cartItem: CartItem) => {
        const item = reduxState.items.itemMap[cartItem.itemId];
        const groupedItem =
          reduxState.items.groupedItemMap[cartItem.groupedItemId];
        const itemColor = groupedItem.itemColors.find(
          (itemColor: ItemColor) => itemColor.id === cartItem.itemColorId,
        )!;
  ...
  const totalPrice = populatedCartItems.reduce(
    (acc: number, populatedCartItem: PopulatedCartItem) =>
      acc + populatedCartItem.item.price * populatedCartItem.quantity,
    0,
  );
        ...
        <div>
          <ul className="mb-6">
            {populatedCartItems.map((populatedCartItem) => (
              <li
                key={populatedCartItem.item.id}
                className="flex justify-between items-center mb-4"
              >
                <div>
                  <p className="font-medium">
                    {populatedCartItem.groupedItem.id}
                  </p>
                  <p className="font-medium">
                    {populatedCartItem.itemColor.color}{" "}
                    {populatedCartItem.item.size}
                  </p>
                  <p className="text-gray-500">
                    {populatedCartItem.quantity} x $
                    {populatedCartItem.item.price.toFixed(2)}
                  </p>
                </div>
