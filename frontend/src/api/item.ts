...
const itemGenerator = (index: number): GroupedItem => {
  ...
  const itemColors = getRandomSubarray(colorList).map(
    (color: Color, colorIndex: number) => ({
      id: `${index}-${colorIndex}`,
      color,
      groupedItemId: id,
      imgUrls: getRandomSubarray(imgUrlList),
      items: getRandomSubarray(sizeList).map(
        (size: ItemSize, sizeIndex: number) => ({
          size,
          price: getRandomNumber(),
          inventory: getRandomNumber(),
          id: `${index}-${colorIndex}-${sizeIndex}`,
          itemColorId: `${index}-${colorIndex}`,
        }),
      ),
    }),
  );
...
const getGroupedItems = async () => {
  ...
  const response: AxiosResponse = await axios.get(
    "http://localhost:3000/grouped-items",
  );
