  ...
  reducers: {
    ...
    deleteItemFromCart: (state, action: PayloadAction<string>) => {
      state.cartItems = state.cartItems.filter(
        (cartItem: CartItem) => cartItem.itemId !== action.payload,
      );
    },
    upsertItemFromCart(state, action: PayloadAction<CartItem>) {
      const upsertedCartItem = action.payload;
      const index = state.cartItems.findIndex(
        (cartItem: CartItem) => cartItem.itemId === upsertedCartItem.itemId,
      );
...
export const { clearCart, deleteItemFromCart, upsertItemFromCart } =
  cartSlice.actions;
